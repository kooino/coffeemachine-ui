...
function App() {
  ...
  const confirmValg = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.status === "Valg gemt") setShowPopup(true);
      else setFejl("Kunne ikke gemme valg.");
    } catch (error) {
  ...
  return (
    <div className="App">
      <div className="container">
        ...
        {showPopup && <SuccessPopup message={`✅ Valg gemt: ${valg}`} onClose={() => setShowPopup(false)} />}
        ...
        <h3>Status:</h3>
        <p>{kortOK ? "✅ Kort godkendt!" : uid ? "❌ Kort ikke godkendt endnu!" : "Indtast UID og scan kort"}</p>
