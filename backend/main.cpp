...
int main() {
    ...
    while (true) {
        ...
        if (request.find("POST /gem-valg") != std::string::npos) {
            ...
            skrivTilFil("uid.txt", uid); // gem UID separat til nulstilling
            if (kortOK) responseBody = "{\"kortOK\": true, \"uid\": \"" + uid + "\"}";
            else responseBody = "{\"kortOK\": false, \"error\": \"❌ Forkert kort\", \"uid\": \"" + uid + "\"}";
